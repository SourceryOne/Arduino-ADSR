...
void envelopeWrite2() {
  ...
  if (elapsedTime2 < rampDuration2) {
    if (envelope2Phase==1) { 
      ...
      currentValue2 = maxValue * pow(progress, attack2Exponent); }
    if (envelope2Phase==2) { 
      float progress = (float)elapsedTime2 / rampDuration2; 
      currentValue2 = sustainValueCh2*sustainMultiplier + (maxValue - sustainValueCh2*sustainMultiplier) * pow(1.0 - progress, decay2Exponent); }
    if (envelope2Phase==3) { currentValue2=sustainValueCh2*sustainMultiplier; }
    if (envelope2Phase==4) { 
      float progress = (float)elapsedTime2 / rampDuration2; 
      currentValue2 = (sustainValueCh2*sustainMultiplier) * pow(1.0 - progress, release2Exponent); }
  } else { //ENVELOPE PHASE TIME IS UP!
    if (envelope2Phase==1) {
      currentValue2 = maxValue;  // Ensure it stays at maxValue after 1000ms OR SHOULD IT BE THE TOP OF SUSTAIN IF DECAY IS OFF!
